...
6. VERY IMPORTANT: When matching free-text topics (e.g. "authentication failed" or "sudo"), DO NOT restrict your search to a single narrow field like `data.audit.command` or `rule.description`. Instead, use `query_string` and explicitly provide an array of broad fields in the `fields` parameter (e.g. `{"query_string": {"fields": ["rule.description", "full_log", "rule.groups"], "query": "*authentication failed*", "lenient": true}}`). ALWAYS prioritize searching Wazuh-native fields like `rule.groups` and `rule.description`. CRITICAL: Whenever you use `query_string`, you MUST include `"lenient": true`. CRITICAL: NEVER include `data.*` in the `fields` array! Expanding `data.*` will crash OpenSearch with a "too_many_nested_clauses" exception. Use `full_log` as a safe fallback instead.
